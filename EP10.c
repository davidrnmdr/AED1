...
// Function to find the number of steps taken to go from vertex A to vertex B without passing through vertex C
int countSteps(int graph[MAX_VERTICES][MAX_VERTICES], int src, int dest, int avoid, int vertices) {
...
int main() {
    ...
    while (scanf("%d %d", &vertices, &paths) != EOF) {
        ...
        int stepCount = countSteps(graph, src, dest, avoid, vertices);
        printf("%d\n", stepCount);
    }
